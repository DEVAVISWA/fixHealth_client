...
import { getBookingByID, getPatientList, getPhysioList } from "../requests/dataRequest";
...
function Sales() {  
  ...
  const handleSelectPatient = async (event) => {
    setPhysio(event.target.value);
    dispatch(showBackDrop());
    const { data, status } = await getBookingByID(event.target.value);
    if (status) {
  ...
  useEffect(() => {
    ...
    const getPatientsDetails = async () => {
      const { data, status } = await getPatientList();
      if (status) {
        setPatients(data);
        setHeaders(
          bookingHeaders({
        ...
        <Box display={"flex"} gap={2} alignItems={"center"}>
          ...
          <Box sx={{ minWidth: 120 }}>
            <FormControl sx={{ m: 1, minWidth: 120 }} size="small">
              <InputLabel id="physio-select-label">Patient</InputLabel>
              <Select
